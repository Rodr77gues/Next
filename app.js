...
// ====== CARREGAR ESTAÇÕES DO estacoes.json ======
fetch("estacoes.json")
  .then(res => res.json())
  .then(data => {
    linhas = data;
...
// ====== ATUALIZAR ESTAÇÕES QUANDO MUDA A LINHA ======
if (linhaSelect) {
...
function saveRoute() {
  ...
  userRoute = { lineName, entrada, saida };
  localStorage.setItem("nextstopRoute", JSON.stringify(userRoute));

  routeStatus.style.color = "#b5ffb8";
  routeStatus.textContent = `Rota salva: ${entrada} ➝ ${saida} (${lineName})`;
}
...
function ativarLocalizacao() {
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      setTimeout(() => {
        routeStatus.innerHTML += "<br>✅ 3 pessoas encontradas próximas!";
      }, 2000);
    },
    err => {
      console.error(err);
...
// ====== FILTRAR PERFIS (com base no objetivo) ======
...
// ====== PERFIL (TOPO + ABA PERFIL) ======
function renderTopUser() {
  if (!currentUser) return;
  topUser.innerHTML = "";
  const img = document.createElement("img");
  img.src =
    currentUser.foto || "https://via.placeholder.com/60x60.png?text=NS";
...
function renderProfileBox() {
  ...
  profileBox.innerHTML = `
    <div class="profile-box-header">
      <img src="${currentUser.foto || "https://via.placeholder.com/80x80.png?text=NS"}" alt="Foto de perfil" />
      <div>
        <strong>${currentUser.nome}</strong><br/>
        <span style="font-size:12px; opacity:0.8;">${currentUser.email}</span><br/>
        <span style="font-size:12px;">Objetivo: ${currentUser.objetivo}</span>
      </div>
    </div>
...
// ====== INICIALIZAÇÃO ======
function initApp() {
  // se as linhas já tiverem carregado do estacoes.json, o populateLines já foi chamado
